<header x-data="{ open: false }" class="bg-blue-900 text-white p-4">
            ...
            @auth
                ...
                <div class="flex items-center space-x-3">
                    <a href="{{ route('dashboard') }}">
                        <img src="{{ Auth::user()->avatar ? asset('storage/' . Auth::user()->avatar) : 'https://ui-avatars.com/api/?name=' . urlencode(Auth::user()->name) . '&background=0D8ABC&color=fff&size=128&rounded=true' }}"
    ...
    <!-- Mobile Menu -->
    <nav x-show="open" class="md:hidden bg-blue-900 text-white mt-5 pb-4 space-y-2">
